...
router.post("/balance/:id", async (req, res) => {
    const { balance, items, status, updatedById, curr_class, session, term } = req.body;
    const id = req.params.id.substring(0, 7);
...
router.post("/reversal/:id", async (req, res) => {
    const { status, toReverse, updatedById, curr_class, session, term } = req.body;
    const id = req.params.id.substring(0, 7);
...
router.post("/create", async (req, res) => {
    const { total, balance, items, status, studentId, createdById, curr_class, session, term, } = req.body;
    const Items = items.map((item) => {
        return {
            id: nanoid(8),
            feeId: item?.feeId,
